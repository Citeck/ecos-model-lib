...
class WorkspaceServiceImpl @JvmOverloads constructor(
    services: ModelServiceFactory,
    ...
    companion object {
        ...
        /**
         * System id generated by the model app for a workspace which doesn't have one of its own.
         * It is a valid answer, but not a mapping to a real system id, so it must not be cached
         * as a resolved one.
         */
        private const val DELETED_WS_SYS_ID_PREFIX = "DELETED_"

        private const val ID_MAPPING_CACHE_SIZE = 1000L
        private val ID_MAPPING_ACCESS_TTL: Duration = Duration.ofSeconds(30)
        private val ID_MAPPING_RESOLVED_TTL: Duration = Duration.ofMinutes(5)

        /**
         * Mappings which resolved nothing are re-checked quickly, see [WsIdMappingCache].
         */
        private val ID_MAPPING_UNRESOLVED_TTL: Duration = Duration.ofSeconds(30)
    }
    ...
    private val workspaceSysIdCache = createIdMappingCache(
        WorkspaceApi.IdMappingType.WS_ID_TO_SYS_ID,
        ticker
    ) { sysId -> sysId.isNotBlank() && !sysId.startsWith(DELETED_WS_SYS_ID_PREFIX) }

    private val workspaceIdBySysIdCache = createIdMappingCache(
        WorkspaceApi.IdMappingType.WS_SYS_ID_TO_ID,
        ticker
    ) { workspaceId -> workspaceId.isNotBlank() }

    private fun createIdMappingCache(
        mappingType: WorkspaceApi.IdMappingType,
        ticker: Ticker,
        isResolved: (String) -> Boolean
    ): WsIdMappingCache {
        return WsIdMappingCache.create(
            resolvedTtl = ID_MAPPING_RESOLVED_TTL,
            unresolvedTtl = ID_MAPPING_UNRESOLVED_TTL,
            accessTtl = ID_MAPPING_ACCESS_TTL,
            maxSize = ID_MAPPING_CACHE_SIZE,
            isResolved = isResolved,
            ticker = ticker
        ) { workspaceApi.mapIdentifiers(listOf(it), mappingType)[0] }
    }
